  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->logger('ptalk_message')->notice('@type: message @mid of the thread @tid has been restored @state.',
      [
        ...
        '@state' => is_null($account) ? t('for all participants') : t('for') . ' ' . $this->currentUser->getAccountName(),
      ]);

    if (is_null($account)) {
      $this->messenger()->addMessage($this->t('Message has been restored for all users.'));
    }
    else {
      $this->messenger()->addMessage($this->t('Message has been restored.'));
    }
  }
